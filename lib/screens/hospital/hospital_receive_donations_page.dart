...
class HospitalReceiveDonationsPage extends StatefulWidget {
  const HospitalReceiveDonationsPage({super.key});

  @override
  State<HospitalReceiveDonationsPage> createState() => _HospitalReceiveDonationsPageState();
}

class _HospitalReceiveDonationsPageState extends State<HospitalReceiveDonationsPage> {
  ...
  Future<void> _saveEntry() async {
    ...
    try {
      ...
      // Calculate expiry (e.g. 42 days for Whole Blood)
      final collectionDate = _dateController.text.isEmpty 
            ? DateTime.now() 
            : DateTime.parse(_dateController.text);
      final expiryDate = collectionDate.add(const Duration(days: 42));
      ...
      await ApiService().addHospitalStock(user.id!, data);
  ...
  Widget _buildQuickEntryCard() {
    return Container(
      ...
      child: Column(
        children: [
          // Removed Scan button and OR text
